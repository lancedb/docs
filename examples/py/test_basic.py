...
def test_quickstart(tmp_path):
    ...
    # --8<-- [end:set_uri]
    uri = tmp_path
    # --8<-- [start:connect_basic]
    db = lancedb.connect(uri)
    # --8<-- [end:connect_basic]

    # --8<-- [start:create_table_basic]
    data = [
    ...
    # --8<-- [end:create_table_basic]

    # --8<-- [start:create_table_pandas_basic]
    df = pd.DataFrame(
        [
            {"vector": [3.1, 4.1], "item": "foo", "price": 10.0},
            {"vector": [5.9, 26.5], "item": "bar", "price": 20.0},
    ...
    # --8<-- [end:create_table_pandas_basic]

    # --8<-- [start:create_empty_table_basic]
    schema = pa.schema([pa.field("vector", pa.list_(pa.float32(), list_size=2))])
    tbl = db.create_table("empty_table", schema=schema)
    # --8<-- [end:create_empty_table_basic]
    # --8<-- [start:open_table]
    ...
    tbl.add(data)
    # --8<-- [end:add_data]
    # --8<-- [start:vector_search_basic]
    tbl.search([100, 100]).limit(2).to_pandas()
    # --8<-- [end:vector_search_basic]
    tbl.add(
        [
...
async def test_quickstart_async(tmp_path):
    uri = tmp_path
    # --8<-- [start:connect_async_basic]
    db = await lancedb.connect_async(uri)
    # --8<-- [end:connect_async_basic]
    # --8<-- [start:create_table_async_basic]
    data = [
        {"vector": [3.1, 4.1], "item": "foo", "price": 10.0},
        {"vector": [5.9, 26.5], "item": "bar", "price": 20.0},
    ]
    ...
    # --8<-- [end:create_table_async_basic]
    # --8<-- [start:create_table_async_pandas_basic]
    df = pd.DataFrame(
        [
            {"vector": [3.1, 4.1], "item": "foo", "price": 10.0},
            {"vector": [5.9, 26.5], "item": "bar", "price": 20.0},
    ...
    # --8<-- [end:create_table_async_pandas_basic]
    # --8<-- [start:create_empty_table_async_basic]
    schema = pa.schema([pa.field("vector", pa.list_(pa.float32(), list_size=2))])
    tbl = await db.create_table("empty_table_async", schema=schema)
    # --8<-- [end:create_empty_table_async_basic]
    # --8<-- [start:open_table_async_basic]
    tbl = await db.open_table("my_table_async")
    # --8<-- [end:open_table_async_basic]
    # --8<-- [start:table_names_async_basic]
    print(await db.table_names())
    # --8<-- [end:table_names_async_basic]
    # --8<-- [start:add_data_async_basic]
    # Option 1: Add a list of dicts to a table
    data = [
        {"vector": [1.3, 1.4], "item": "fizz", "price": 100.0},
        {"vector": [9.5, 56.2], "item": "buzz", "price": 200.0},
