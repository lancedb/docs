...
def validate(config: Config, resolved: Resolved, docs_json: dict) -> list[str]:
    ...
    # The REST reference is generated by Mintlify from the synced spec, so a
    # navigation that never names it renders a group with no operations in it —
    # a silently empty reference rather than a build failure.
    if config.openapi:
        # `dest` is relative to this repository; the navigation names the spec
        # relative to the site root, so compare them in the same frame.
        dest = Path(config.openapi["dest"]).resolve()
        site_rel = next(
            (
                dest.relative_to(root.path.resolve()).as_posix()
                for root in config.roots
                if dest.is_relative_to(root.path.resolve())
            ),
            dest.name,
        )
        if site_rel not in json.dumps(docs_json):
            raise AssembleError(
                f"navigation never references the OpenAPI spec at {site_rel}; "
                "the REST reference would render empty"
            )
