...
## Using FTS Indexes with Async Connections in LanceDB OSS
...
When using `connect_async()` in LanceDB OSS, create FTS indexes using `create_index` with the `FTS` configuration:
