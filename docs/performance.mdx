...
LanceDB uses separate pools for compute work and storage I/O. On a container with two visible CPUs, current releases intentionally run one compute worker by default. No manual configuration is needed.

If every query logs an I/O core reservation warning on a two-CPU container, upgrade from LanceDB `0.21.1` or earlier. The warning fired on the intended fallback and is fixed in newer releases.

Two environment variables control different resources, and are commonly confused:

| Variable | Controls | Default behavior |
|----------|----------|------------------|
| `LANCE_CPU_THREADS` | Number of compute workers. | Sized from visible CPUs. Set to `1` for a two-CPU container if an explicit override is needed. |
| `LANCE_IO_THREADS` | Concurrent storage operations, not reserved CPU cores. | Can exceed the CPU count because I/O workers spend most of their time waiting on storage. |

Keep the defaults unless measurements show that the workload benefits from an override. For the current defaults and tuning guidance, see the [Lance threading model](https://lance.org/guide/performance/#threading-model).
