...
LanceDB Enterprise is built for workloads that are too large or too busy for a single machine. We have active users
in production with datasets containing billions of rows (for search, training and analytics workloads). Instead of asking your
...
- **Distributed search**: <Badge color="purple">Coming soon</Badge>. Enterprise is expanding search execution across more of the cluster to support
  larger fan-out workloads and higher overall throughput.
