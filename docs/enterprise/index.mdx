...
This section summarizes the core capabilities LanceDB Enterprise adds over those available in OSS.
...
[It's very simple to get started with OSS](/quickstart/): Immediately get started with `pip install lancedb` and begin ingesting your data and vectors into LanceDB. LanceDB OSS makes sense when your entire dataset fits on one machine, daily traffic remains under fifty queries per second, and you are able to run manual maintenance (compaction, reindexing, etc.) without affecting users. 
