...
processed, `StreamingDataset` reads subsequent batches. The `prefetch_batches` parameter specifies the number of batches simultaneously kept in flight split (default = `4`). A larger value provides more buffering against jittery workloads, but also increases memory use and I/O concurrency.
