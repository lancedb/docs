...
- **Train from object storage while keeping your GPUs fed.** Lance reads random frames straight from S3, GCS, the HF Hub or HF Storage Buckets, fetching only the bytes each batch needs, so every batch can be a global shuffle: any frame, from anywhere in the dataset. Upstream gives you two ways to train: download the dataset and shuffle from local disk, or stream it from the Hub and shuffle inside a window of buffered frames. The left half of the table gives Lance the exact batches the streaming reader produced. The right half is the global shuffle, Lance with nothing downloaded against upstream on local NVMe after the download. Samples/s, 8 workers, batch 64, mean of two runs. RAM is the whole loader:
...
The shuffle window is 0.05% of the dataset, at least 5,000 frames: 5,000 on the small sets, 13,815 on DROID. Streaming DROID at that window needs 122 GB, so those two runs used a 256 GB machine with the same decode speed. The DROID pair is noisy: Lance 684 and 1,124, streaming 303 and 653.

Two things to take from this. Given the same batches, Lance from an object store is 2x faster than upstream streaming on DROID and 5x to 15x on the smaller sets, at a fraction of the RAM: 8 GB against 122 GB on DROID. The window is why the gap is this wide: at upstream's default 1,000 frames the same comparison is 1.6x on DROID and 5x to 12x elsewhere, and widening it for a better shuffle makes the streaming reader decode more of each file out of order and buffer more decoded frames in every worker, while for Lance a wider window is only a different read order. And the global shuffle, the one you actually want, runs from object storage at local-disk speed: Lance with nothing downloaded beats upstream reading local NVMe on every dataset here, 1.1x to 1.7x. Three more public datasets we converted, berkeley_rpt, toto and soarm101, are left out because their metadata lists frames the videos do not have, so one reader or another raises on them. `lerobot-lance-doctor` reports that before you train.
