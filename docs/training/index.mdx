...
Training robot policies with [LeRobot](https://huggingface.co/docs/lerobot/index)? It reads Lance datasets natively, so the same tables feed `LeRobotDataset` directly. See the [LeRobot guide](/integrations/lerobotdataset).
