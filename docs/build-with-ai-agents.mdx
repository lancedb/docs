...
Suppose `nomic-embed-text` wins. Ask the agent to apply that result:

```text Agent prompt
The `embed-nomic` experiment won. Use the lancedb skill to apply
`vector_nomic` to `main`.

- On LanceDB OSS, rerun the validated Nomic embedding transformation against
  the main table in batches, then call `table.optimize()`.
- On LanceDB Enterprise, inspect `table.branches.diff("embed-nomic")`. If the
  branch satisfies the added-column merge requirements, preview with
  `merge(..., dry_run=True)` and merge only after the preview is ready.
  Otherwise, rerun the validated transformation on `main`.
- Verify a bounded vector search on `main`.
- Keep the losing branch until I explicitly approve its deletion.
```

Enterprise branch merge promotes eligible new columns, but it does not replay
row updates, index changes, or a backfill that spans several commits. With OSS,
rerun the chosen operation on `main`. See
[Branches](/tables/branching), especially
[Compare and merge a branch into `main`](/tables/branching#compare-and-merge-a-branch-into-main),
for the exact rules.
...
On LanceDB Enterprise, eligible added-column changes can be reviewed and merged from a
branch. See [Branches](/tables/branching) for more information on branching and merge rules.
