...
clips = db.create_materialized_view(
    "clips",
    query=videos.search(None).select(["video_path", "duration", "metadata"]),
    udtf=extract_clips,
)
...
### Inheriting all source columns

```python
# Inherit all columns from the source table
clips = db.create_materialized_view(
    "clips",
    query=videos.search(None),  # No .select() — all columns inherited
    udtf=extract_clips,
)
```
...
| video_path | duration | metadata | clip_start | clip_end | clip_bytes |
|------------|----------|----------|------------|----------|------------|
| /v/a.mp4 | 120.0 | \{fps: 30\} | 0.0 | 10.0 | b"\x00\x1a..." |
| /v/a.mp4 | 120.0 | \{fps: 30\} | 10.0 | 20.0 | b"\x00\x2b..." |
| /v/a.mp4 | 120.0 | \{fps: 30\} | 20.0 | 30.0 | b"\x00\x3c..." |
| /v/b.mp4 | 60.0 | \{fps: 24\} | 0.0 | 10.0 | b"\x00\x4d..." |
| /v/b.mp4 | 60.0 | \{fps: 24\} | 10.0 | 20.0 | b"\x00\x5e..." |

Each child row in `clips` carries `video_path`, `duration`, and `metadata` from its parent in `videos`, plus the UDTF-generated `clip_start`, `clip_end`, and `clip_bytes` columns.
...
## Batch UDTFs vs Scalar UDTFs

| | Scalar UDTF | Batch UDTF |
|---|---|---|
| **Decorator** | `@scalar_udtf` | `@geneva.udtf` |
| **Cardinality** | 1:N (per-row expansion) | N:M (arbitrary) |
| **Input** | Column values for one row | `GenevaQueryBuilder` over entire table/partition |
| **Refresh** | Incremental (only new rows) | Full (re-reads entire source) |
| **Parallelism** | Fragment-parallel (automatic) | Partition-parallel (via `partition_by`) |
| **Inherited columns** | Automatic from query | Not applicable — output schema is independent |
| **Use cases** | Chunking, splitting, tiling | Deduplication, clustering, aggregation |

For N:M transformations that need cross-row visibility, see [Batch UDTFs](/geneva/udfs/batch-udtfs).
