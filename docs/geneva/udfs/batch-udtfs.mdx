...
Use the `@geneva.udtf` decorator on a class or function. The UDTF receives a query builder over the source data and **yields** `pa.RecordBatch` objects with an arbitrary output schema.
...
```python
@geneva.udtf(output_schema=pa.schema([
