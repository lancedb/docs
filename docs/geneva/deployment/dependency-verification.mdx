...
## Why Dependency Mismatches Matter
...
| Unexpected behavior, wrong results | Silent version incompatibilities |
...
# Compare and print (requires Ray to be initialized, e.g., in Geneva context)
result = compare_ray_environments()

# Data-only mode (no printing)
from geneva.runners.ray.compare_env import compare_environments
result = compare_environments()

# Auto-init and connect to existing cluster
result = compare_ray_environments(auto_init=True)

# Auto-init a new local cluster for testing
...
For detailed API documentation on the environment comparison functions, see the [Geneva Diagnostics API Reference](https://docs.lancedb.com/geneva/api/diagnostics).

## Related Documentation

- [Troubleshooting](/geneva/deployment/troubleshooting) - General troubleshooting for Geneva deployments
- [Performance](/geneva/jobs/performance) - Resource tuning
- [Execution Contexts](/geneva/jobs/contexts) - Managing execution environments
