...
LanceDB provides a comprehensive suite of indexes for different data types and use cases:
...
(note: acknowledge pq in here somewhere) test

An IVF is an index that facilitates rapid nearest neighbor searches by drastically reducing the search space.
