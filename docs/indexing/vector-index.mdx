...
| `num_partitions` | When left unset, targets roughly `sqrt(num_rows)`. |
| `target_partition_size` | An alternative IVF sizing knob that derives the partition count by setting the number of rows per partition. Defaults to `8192 = 2^13` for IVF-family indexes and `1,048,576 = 2^20` for IVF-HNSW-family indexes. `num_partitions` takes precedence over `target_partition_size`. |
...
There are also advanced search-time controls for thresholded retrieval, recall measurement, and working around index-level metric constraints.
...
<Warning>
Be careful to use the same distance metric during index creation and search. 
</Warning>
