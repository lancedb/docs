...
| Maximum compression | `IVF_RQ` | RaBitQ-style quantization with very strong compression. Requires `dimension % 8 == 0`. | Around `1/32` of raw size |
...
- `IVF_RQ`
  ...
  - Vector `dimension` must be divisible by `8`.
- `IVF_PQ`
...
- Index Types:
    ...
    - `IVF_RQ`: best compression for large, high-dimensional datasets (`dimension` must be divisible by `8`)
