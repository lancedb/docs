...
<Warning>
In LanceDB releases before this behavior shipped, a second `where(...)` call silently replaced the first filter, so only the last predicate was applied. If you are upgrading from an older release and previously relied on that behavior, review any code that chains `where(...)` and drop the earlier calls you no longer want to apply.
</Warning>
