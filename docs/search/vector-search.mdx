...
For deeper tuning guidance on indexing and performance estimation, see [Vector Indexes](/indexing/vector-index/#search-configuration) and [FAQ: Do I need to set a refine factor when using an index?](/faq/faq-oss#do-i-need-to-build-a-vector-index-to-run-vector-search),
and for tuning `nprobes`, see below.
