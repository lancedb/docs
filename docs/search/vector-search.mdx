...
index and falls back to an exhaustive (brute-force) search. On LanceDB Enterprise,
a safety limit rejects brute-force vector searches over large tables, so a metric
mismatch can return an error instead of running the exhaustive search.
