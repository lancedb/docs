...
For deeper tuning guidance on indexing and performance estimation, see [Vector Indexes](/indexing/vector-index/#search-configuration) and [FAQ: Do I need to set a refine factor when using an index?](/faq/faq-oss/#do-i-need-to-set-a-refine-factor-when-using-an-index),
