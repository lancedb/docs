...
- **Train from object storage while keeping your GPUs fed.** Lance reads random frames straight from S3, GCS, the HF Hub or HF Storage Buckets, fetching only the bytes each batch needs, so every batch is a global shuffle: any frame, from anywhere in the dataset. It plans the whole batch at once (one table read, one blob fetch, decoders reused) instead of fetching one frame per call, which is why 4 dataloader workers per GPU keep 8xH100 fed where upstream needs 8. Throughput in samples/s, 8 workers, batch 64. Lance and upstream local both draw every batch from the whole dataset; upstream streaming does a reservoir shuffle, reading the dataset in order and drawing each batch from a pool of recently seen frames, so those two columns are not the same job:
...
**Feature engineering costs nothing until you use it.** With [Geneva](/geneva/udfs/udfs), a new column is a declaration: `add_columns` registers a Python function over existing columns and computes nothing; `backfill` materializes it in parallel when you want it. No copy of the dataset, and the videos are never touched:
