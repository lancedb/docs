...
description: "Commonly asked questions about LanceDB Enterprise."
icon: "server"
mode: wide
---
...
### Architecture and Fault Tolerance {#architecture-and-fault-tolerance}

#### What is the impact of losing each component (query node, indexer, etc.) in the LanceDB stack? {#what-is-the-impact-of-losing-each-component-query-node-indexer-etc-in-the-lancedb-stack}
...
#### What does plan executor cache versus not cache? {#what-does-plan-executor-cache-versus-not-cache}
The plan executor caches the table data, not the table indices.

#### Should I use disk cache or memory cache for the plan executor? {#should-i-use-disk-cache-or-memory-cache-for-the-plan-executor}
...
#### How is the PE (Plan Executor) fleet shared? What fault tolerance exists (how many nodes can be lost)? {#how-is-the-pe-plan-executor-fleet-shared-what-fault-tolerance-exists-how-many-nodes-can-be-lost-}
...
#### How is strong and weak consistency configured in the enterprise stack? {#how-is-strong-and-weak-consistency-configured-in-the-enterprise-stack}
...
#### Can I use GPU for indexing? {#can-i-use-gpu-for-indexing}
...
#### What are the parameters that can be configured for my LanceDB cluster? {#what-are-the-parameters-that-can-be-configured-for-my-lancedb-cluster}
...
#### What are the metrics that LanceDB exposes for monitoring? {#what-are-the-metrics-that-lancedb-exposes-for-monitoring}
...
#### How do I integrate LanceDB monitoring metrics with my monitoring dashboard? {#how-do-i-integrate-lancedb-monitoring-metrics-with-my-monitoring-dashboard}
